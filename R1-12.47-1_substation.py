...
def start_loadshed(argv):
  ...
  while grantedTime <= simTime:
    ...
    for ind in range (0, endpoint_count):
      ...
      if epName.split('/')[0] == fedName and 'substation' in epName.split('/')[1]:
        ...
        for epKey in epMessages.keys():
          logger.info(f'\tsource: {epMessages[epKey]["source"]}, data: {epMessages[epKey]["data"]}, time received: {epMessages[epKey]["time received"]}')
          if 'mtr' in epMessages[epKey]['source']:
            totalBillMtrLoad += float(epMessages[epKey]["data"].split(" ")[0])
      if int(currTime) in loadshedScenario.keys():
        for key in loadshedScenario[int(currTime)]:
          destination = f'R1-12.47-1/{key}-EP'
          messg = loadshedScenario[int(currTime)][key]
          h.helicsEndpointSendBytesTo(fedEP, 'OUT_OF_SERVICE', destination)
    ...
    logger.info(f'\tCurrent system load at billing meter level: {totalBillMtrLoad} W')
